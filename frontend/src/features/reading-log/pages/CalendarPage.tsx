...
import { Flame, ChevronLeft, ChevronRight, BookOpen, Clock, CalendarCheck, BookMarked } from 'lucide-react'
...
interface DayLog { date: string; pagesRead: number; minutesRead: number }
...
export function CalendarPage() {
  ...
  const load = async () => {
    ...
    if (libRes.data.length > 0 && !selectedBook) setSelectedBook(libRes.data[0].id)
  }
      ...
      <div className={styles.layout}>
        <div className={styles.calendarSection}>
          <div className={styles.streakBanner}>
            <Flame size={48} color="white" strokeWidth={1.5} />
            <div>
        ...
        <div className={styles.logSection}>
          ...
          <div className={styles.logForm}>
            <div className={styles.field}>
              <label>Livro</label>
              <select className={styles.select} value={selectedBook} onChange={(e) => setSelectedBook(e.target.value)}>
                {library.length === 0 && <option value="">Nenhum livro em leitura</option>}
                {library.map((ub) => <option key={ub.id} value={ub.id}>{ub.book.title}</option>)}
              </select>
            </div>
            <div className={styles.field}>
              <label>Data</label>
              <input type="date" className={styles.input} value={logDate} onChange={(e) => setLogDate(e.target.value)} max={today.toISOString().split('T')[0]} />
            </div>
            <div className={styles.field}>
              <label>Páginas lidas</label>
              <input type="number" className={styles.input} value={pages} onChange={(e) => setPages(e.target.value)} placeholder="0" min={0} />
            </div>
            <div className={styles.field}>
              <label>Minutos de leitura</label>
              <input type="number" className={styles.input} value={minutes} onChange={(e) => setMinutes(e.target.value)} placeholder="0" min={0} />
            </div>
            <Button fullWidth onClick={logSession} loading={saving} disabled={!selectedBook}>
              <CalendarCheck size={15} /> Registrar Sessão
            </Button>
          </div>
