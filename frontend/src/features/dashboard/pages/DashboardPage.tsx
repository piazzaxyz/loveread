...
export function DashboardPage() {
  ...
  const genreData = Object.entries(data.allGenres)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 7)
    .map(([name, value]) => ({ name, value }))

  const leaderboardData = data.leaderboard.map((u) => ({
    name: u.name,
    lidos: u.booksRead,
    páginas: u.totalPages,
  }))
  ...
  return (
    <div className={styles.page}>
      ...
      {/* Leaderboard */}
      <section>
        <h2 className={styles.sectionTitle}>🏆 Ranking de Leitores</h2>
      ...
      <div className={styles.chartsRow}>
        {genreData.length > 0 && (
          <Card className={styles.chartCard}>
            <h3 className={styles.chartTitle}>📚 Gêneros Mais Lidos</h3>
        ...
        {leaderboardData.length > 0 && (
          <Card className={styles.chartCard}>
            <h3 className={styles.chartTitle}>📖 Livros Lidos por Pessoa</h3>
            <ResponsiveContainer width="100%" height={220}>
                ...
                <div key={log.id} className={styles.activityItem}>
                  <div className={styles.activityAvatar}>
                    {log.user.avatarUrl ? (
                      ...
                      <span>{log.user.name[0].toUpperCase()}</span>
                    )}
                  </div>
                  <div className={styles.activityInfo}>
                    <span className={styles.activityText}>
                      <strong>{log.user.name}</strong> leu{' '}
                      <span className={styles.bookName}>{log.userBook.book.title}</span>
                    </span>
...
function LeaderboardCard({ user, rank }: { user: UserStats; rank: number }) {
  const RANK_EMOJI = ['🥇', '🥈', '🥉']
  const rankEmoji = RANK_EMOJI[rank - 1] || `#${rank}`

  return (
    <Card className={styles.leaderCard}>
      <div className={styles.rankBadge}>{rankEmoji}</div>
      ...
      <div className={styles.leaderStats}>
        ...
        <div className={styles.leaderStat}>
          <span className={styles.leaderStatVal}>{user.streak}🔥</span>
          <span className={styles.leaderStatLabel}>Streak</span>
        </div>
      </div>

      {user.favoriteGenre && (
        <span className={styles.favoriteGenre}>❤️ {user.favoriteGenre}</span>
      )}

      {user.booksReading > 0 && (
        <span className={styles.readingNow}>📖 Lendo {user.booksReading} livro{user.booksReading > 1 ? 's' : ''}</span>
      )}
    </Card>
  )
}
